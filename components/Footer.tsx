...
export default function Footer() {
  ...
  const socials = [
    ...
    {
      label: 'Instagram',
      href: 'https://www.instagram.com/gerald_devs/',
      icon: (
        <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4">
  ...
  return (
    <footer className="bg-neu-bg py-12 mt-0">
      <div className="max-w-6xl mx-auto px-6">
        <div
          className="rounded-2xl p-8"
          style={{ boxShadow: '8px 8px 16px #b8bec8, -8px -8px 16px #ffffff' }}
        >
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            {/* Brand */}
            <div className="text-center md:text-left">
              <p className="font-syne font-bold text-xl text-neu-text">
                Gerald Okeke<span className="gradient-text">.</span>
              </p>
              <p className="text-neu-muted text-sm font-dm mt-1">
                Senior Software Engineer &bull; Malta
              </p>
            </div>
            ...
            <div className="flex items-center gap-6">
              {[
                ...
                <Link
                  key={link.href}
                  href={link.href}
                  className="text-sm font-dm text-neu-muted hover:text-neu-accent transition-colors duration-200"
                >
                  {link.label}
                </Link>
              ))}
            ...
            <div className="flex items-center gap-3">
              {socials.map((s) => (
                <a
                  ...
                  aria-label={s.label}
                  className="w-9 h-9 rounded-xl flex items-center justify-center text-neu-muted hover:text-neu-accent transition-all duration-200"
